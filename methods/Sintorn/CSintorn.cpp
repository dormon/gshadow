...
CSintorn::CSintorn(
    ...
    unsigned ShadowFrustumusPerWorkGroup){
  this->UseUniformTileSizeInClipSpace=false;
  this->UseUniformTileDivisibility   =false;

  //set wavefront size
  this->WavefrontSize=WavefrontSize;
  this->Adjacency=Adjacency;
  this->ShadowFrustaPerWorkGroup=ShadowFrustumusPerWorkGroup;

  //set window size
  for(int k=0;k<2;++k)this->WindowSize[k]=WindowSize[k];

  //compute tiles sizes in tiles
  this->NumLevels=ChooseTileSizes(&this->TileDivisibility,this->WindowSize,this->WavefrontSize);
